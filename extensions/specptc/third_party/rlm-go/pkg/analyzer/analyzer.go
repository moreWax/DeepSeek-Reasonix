// Package analyzer provides context structure pre-analysis for RLM.
// It detects the structure of input context (JSON, markdown, code, etc.)
...
func Analyze(payload any) *ContextAnalysis {
	var content string
	...
	analysis.RecommendedStrategy, analysis.RecommendedChunkSize = determineChunkStrategy(analysis)
...
func determineChunkStrategy(analysis *ContextAnalysis) (ChunkStrategy, int) {
...
// AnalyzeWithConfig performs analysis with custom configuration.
func AnalyzeWithConfig(payload any, cfg Config) *ContextAnalysis {
	analysis := Analyze(payload)

	// Adjust based on configuration
	if cfg.SmallContextThreshold > 0 && analysis.Size < cfg.SmallContextThreshold {
		analysis.RecommendedStrategy = StrategyNone
		analysis.RecommendedChunkSize = 0
	}
