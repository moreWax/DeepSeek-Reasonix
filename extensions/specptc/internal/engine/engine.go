...
func (e *Engine) ObserveSeq(scope Scope, seq uint64, call Call) bool {
	...
	accepted := t.trySend(observeMessage{seq: seq, call: call})
	if !accepted {
		t.markDropped(seq)
	}
	return accepted
...
func (e *Engine) AdvanceSeq(scope Scope, seq uint64) bool {
	...
	accepted := t.trySend(observeMessage{seq: seq, skip: true})
	if !accepted {
		t.markDropped(seq)
	}
	return accepted
...
// Retract cancels the speculative occurrence associated with callID. It is
// used when later streamed code invalidates an earlier partial-program bet.
// Retraction is idempotent and never affects a candidate already claimed by
// authoritative execution.
func (e *Engine) Retract(scope Scope, callID string) bool {
	if callID == "" {
		return false
	}
	t := e.lookup(scope)
	...
	return t.trySend(retractMessage{callID: callID})
}
...
func (e *Engine) claim(ctx context.Context, scope Scope, callID, tool string, arguments []byte) (Handle, bool) {
	...
	var selection claimSelection
	select {
	case <-t.done:
		return "", false
	case selection = <-reply:
	}
	...
	if ctx.Err() != nil {
		t.applyClaimOutcome(selection, false, true)
		return "", false
	}
	select {
	case <-ctx.Done():
	...
	case <-selected.started:
		...
		if selected.startErr != nil || selected.handle == "" {
			if !t.applyClaimOutcome(selection, false, false) {
				return "", false
			}
			return "", false
		}
		if !t.applyClaimOutcome(selection, true, false) {
			return "", false
		}
...
func (t *turn) applyClaimOutcome(selection claimSelection, hit, rollback bool) bool {
	outcome := claimOutcomeMessage{
		entry: selection.entry, callID: selection.callID,
		hit: hit, rollback: rollback, applied: make(chan struct{}),
	}
...
func (e *Engine) Close() error {
	...
	e.cancelWG.Wait()
	e.cancelErrMu.Lock()
	err := errors.Join(e.cancelErrs...)
	e.cancelErrMu.Unlock()
	return err
}
...
func (e *Engine) cancelWorker() {
	...
	for job := range e.stops {
		perAttempt := e.cfg.CancelTimeout / attempts
		if perAttempt <= 0 {
			perAttempt = e.cfg.CancelTimeout
		}
		var lastErr error
		for attempt := 0; attempt < attempts; attempt++ {
			ctx, cancel := context.WithTimeout(context.Background(), perAttempt)
			lastErr = e.backend.Cancel(ctx, job.handle)
			cancel()
			if lastErr == nil {
				break
			}
		...
		if lastErr != nil {
			e.cancelErrMu.Lock()
			e.cancelErrs = append(e.cancelErrs, fmt.Errorf("cancel %q: %w", job.handle, lastErr))
			e.cancelErrMu.Unlock()
		}
	}
}
