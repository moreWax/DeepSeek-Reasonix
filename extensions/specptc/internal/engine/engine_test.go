...
func TestEndReportsStaleWhenActorAlreadyExited(t *testing.T) {
	backend := newFakeBackend()
	scope := testScope("already-exited")
	eng := beginTestEngine(t, backend, Config{}, scope)
	turn := eng.lookup(scope)
	turn.cancel()
	<-turn.done
	_, err := eng.End(context.Background(), scope)
	if !errors.Is(err, ErrStaleScope) {
		t.Fatalf("End error = %v, want ErrStaleScope", err)
	}
}

func TestBarrierFailsPromptlyAfterSaturatedSequenceDrop(t *testing.T) {
	backend := newFakeBackend()
	scope := testScope("dropped-sequence")
	eng := beginTestEngine(t, backend, Config{TurnQueueDepth: 1}, scope)
	turn := eng.lookup(scope)
	if turn == nil {
		t.Fatal("missing turn")
	}
	blockedReply := make(chan claimSelection)
	turn.events <- claimMessage{reply: blockedReply}
	deadline := time.Now().Add(time.Second)
	for len(turn.events) != 0 && time.Now().Before(deadline) {
	...
	if !eng.AdvanceSeq(scope, 1) {
		t.Fatal("first sequence did not fill mailbox")
	}
	if eng.AdvanceSeq(scope, 2) {
		t.Fatal("saturated mailbox unexpectedly accepted second sequence")
	}
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	started := time.Now()
	if eng.Barrier(ctx, scope, 2) {
		t.Fatal("barrier accepted a dropped sequence")
	}
	if time.Since(started) > 100*time.Millisecond {
		t.Fatal("barrier waited despite known dropped sequence")
	}
	<-blockedReply
}

func TestReadyUnclaimedExecutionIsCountedAsWaste(t *testing.T) {
	backend := newFakeBackend()
	scope := testScope("waste")
	eng := beginTestEngine(t, backend, Config{}, scope)
	observe(t, eng, scope, "unused", `{"prompt":"unused"}`, false)
	waitStarts(t, backend, 1)
	if !eng.Complete(context.Background(), scope, "h-unused", CompletionReady) {
		t.Fatal("completion was rejected")
	}
	metrics := endTurn(t, eng, scope)
	if metrics.Wasted != 1 || metrics.Hits != 0 {
...
func TestCancellationWorkerRetriesTransientFailures(t *testing.T) {
	backend := newFakeBackend()
	backend.cancelFailures = 2
	scope := testScope("cancel-retry")
	eng := beginTestEngine(t, backend, Config{CancelWorkers: 1, CancelTimeout: time.Second}, scope)
	observe(t, eng, scope, "retry", `{"n":1}`, false)
	waitStarts(t, backend, 1)
	_ = endTurn(t, eng, scope)
	deadline := time.Now().Add(2 * time.Second)
	for {
		...
		if time.Now().After(deadline) {
			t.Fatalf("cancel attempts = %v, want 3", cancels)
		}
		time.Sleep(time.Millisecond)
	}
}

func TestCloseReportsExhaustedCancellation(t *testing.T) {
	backend := newFakeBackend()
	backend.cancelFailures = 100
	eng, err := New(backend, Config{CancelWorkers: 1, CancelTimeout: 30 * time.Millisecond})
	if err != nil {
		t.Fatal(err)
	}
	scope := testScope("cancel-exhausted")
	if err := eng.Begin(context.Background(), scope); err != nil {
		t.Fatal(err)
	}
	observe(t, eng, scope, "failure", `{"n":1}`, false)
	waitStarts(t, backend, 1)
	_ = endTurn(t, eng, scope)
	if err := eng.Close(); err == nil || !strings.Contains(err.Error(), "cancel") {
		t.Fatalf("Close error = %v, want terminal cancellation failure", err)
	}
	_, cancels := backend.counts()
	if len(cancels) != 3 {
		t.Fatalf("cancel attempts = %v, want 3", cancels)
...
func TestDroppedSequenceDiscardsLaterPendingObservations(t *testing.T) {
	backend := newFakeBackend()
	eng, err := New(backend, Config{TurnQueueDepth: 4})
	if err != nil {
		t.Fatal(err)
	}
	defer eng.Close()
	turn := newTurn(eng, context.Background(), testScope("dropped-pending"))
	turn.pending[3] = observeMessage{seq: 3}
	turn.markDropped(2)
	for seq := uint64(4); seq < 1000; seq++ {
		turn.observeSequenced(observeMessage{seq: seq})
	}
	if len(turn.pending) != 0 {
		t.Fatalf("pending observations retained after drop: %d", len(turn.pending))
	}
}

func TestRollbackPreservesSameEntryReplacementOwnership(t *testing.T) {
	backend := newFakeBackend()
	scope := testScope("same-entry-replacement")
	eng := beginTestEngine(t, backend, Config{}, scope)
	arguments := []byte(`{"prompt":"same"}`)
	observe(t, eng, scope, "same", string(arguments), true)
	observe(t, eng, scope, "other", string(arguments), true)
	waitStarts(t, backend, 1)
	turn := eng.lookup(scope)
	key, err := CanonicalKey("query", arguments)
	if err != nil {
		t.Fatal(err)
	}
	reply := make(chan claimSelection, 1)
	if !turn.send(context.Background(), claimMessage{key: key, callID: "same", reply: reply}) {
		t.Fatal("claim reservation rejected")
	}
	selection := <-reply
	if selection.entry == nil {
		t.Fatal("claim reservation failed")
	}
	select {
	case <-selection.entry.started:
	case <-time.After(time.Second):
		t.Fatal("speculative start did not apply")
	}
	if !eng.Retract(scope, "same") {
		t.Fatal("retraction rejected")
	}
	if !eng.Observe(scope, Call{CallID: "same", Tool: "query", Arguments: arguments, Deterministic: true}) {
		t.Fatal("same-entry replacement rejected")
	}
	fence := make(chan bool, 1)
	if !turn.send(context.Background(), barrierMessage{reply: fence}) || !<-fence {
		t.Fatal("actor fence failed")
	}
	if _, ok := eng.ClaimCall(context.Background(), scope, Call{
		CallID: "same", Tool: "query", Arguments: arguments,
	}); ok {
		t.Fatal("replacement overlapped prior reservation")
	}
	if !turn.applyClaimOutcome(selection, false, true) {
		t.Fatal("rollback was not applied")
	}
	if turn.byCall["same"] != selection.entry {
		t.Fatal("same-entry replacement ownership was lost")
	}
	replacementHandle, ok := eng.ClaimCall(context.Background(), scope, Call{
		CallID: "same", Tool: "query", Arguments: arguments,
	})
	if !ok || replacementHandle != "h-same" {
		t.Fatalf("replacement claim = %q, %v", replacementHandle, ok)
	}
	if !eng.Complete(context.Background(), scope, "h-same", CompletionFailed) {
		t.Fatal("failed completion rejected")
	}
	fence = make(chan bool, 1)
	if !turn.send(context.Background(), barrierMessage{reply: fence}) || !<-fence {
		t.Fatal("completion fence failed")
	}
	if turn.byCall["same"] != nil {
		t.Fatal("candidate removal left stale call ownership")
	}
	if !eng.Observe(scope, Call{CallID: "same", Tool: "query", Arguments: arguments, Deterministic: true}) {
		t.Fatal("re-observation rejected")
	}
	waitStarts(t, backend, 1)
	_ = endTurn(t, eng, scope)
}

func TestRetractDuringReservationSurvivesRollbackAndReplacement(t *testing.T) {
	backend := newFakeBackend()
	scope := testScope("reserved-rollback")
	eng := beginTestEngine(t, backend, Config{}, scope)
	oldArguments := []byte(`{"prompt":"old"}`)
	newArguments := []byte(`{"prompt":"new"}`)
	observe(t, eng, scope, "same", string(oldArguments), false)
	waitStarts(t, backend, 1)
	turn := eng.lookup(scope)
	oldKey, err := CanonicalKey("query", oldArguments)
	if err != nil {
		t.Fatal(err)
	}
	reply := make(chan claimSelection, 1)
	if !turn.send(context.Background(), claimMessage{key: oldKey, callID: "same", reply: reply}) {
		t.Fatal("claim reservation rejected")
	}
	selection := <-reply
	if selection.entry == nil || !eng.Retract(scope, "same") {
		t.Fatal("failed to reserve and retract old occurrence")
	}
	if !eng.Observe(scope, Call{CallID: "same", Tool: "query", Arguments: newArguments}) {
		t.Fatal("replacement observation rejected")
	}
	fence := make(chan bool, 1)
	if !turn.send(context.Background(), barrierMessage{reply: fence}) || !<-fence {
		t.Fatal("actor fence failed")
	}
	replacement := turn.byCall["same"]
	if replacement == nil || replacement == selection.entry {
		t.Fatal("replacement did not supersede reserved occurrence")
	}
	if _, ok := eng.Claim(context.Background(), scope, "query", newArguments); ok {
		t.Fatal("replacement overlapped prior same-ID reservation")
	}
	if !turn.applyClaimOutcome(selection, false, true) {
		t.Fatal("rollback was not applied")
	}
	newKey, err := CanonicalKey("query", newArguments)
	if err != nil {
		t.Fatal(err)
	}
	if turn.byCall["same"] != replacement || replacement.key != newKey {
		t.Fatal("rollback restored retracted occurrence over replacement")
	}
	handle, ok := eng.Claim(context.Background(), scope, "query", newArguments)
	if !ok || handle != "h-same" {
		t.Fatalf("replacement claim after rollback = %q, %v", handle, ok)
	}
	_ = endTurn(t, eng, scope)
}

func TestRetractDoesNotEvictReservedDeterministicClaim(t *testing.T) {
	backend := &lateStartBackend{entered: make(chan struct{}), release: make(chan struct{})}
	eng, err := New(backend, Config{Workers: 1})
	if err != nil {
		t.Fatal(err)
	}
	defer eng.Close()
	scope := testScope("reserved-retract")
	if err := eng.Begin(context.Background(), scope); err != nil {
		t.Fatal(err)
	}
	arguments := []byte(`{"prompt":"same"}`)
	if !eng.Observe(scope, Call{CallID: "first", Tool: "query", Arguments: arguments, Deterministic: true}) ||
		!eng.Observe(scope, Call{CallID: "second", Tool: "query", Arguments: arguments, Deterministic: true}) {
		t.Fatal("observations rejected")
	}
	select {
	case <-backend.entered:
	case <-time.After(time.Second):
	...
	turn := eng.lookup(scope)
	reply := make(chan claimSelection, 1)
	key, keyErr := CanonicalKey("query", arguments)
	if keyErr != nil || !turn.send(context.Background(), claimMessage{key: key, callID: "first", reply: reply}) {
		t.Fatalf("reserve claim: %v", keyErr)
	}
	selection := <-reply
	...
	if !eng.Retract(scope, "first") {
		t.Fatal("retract rejected")
	}
	fence := make(chan bool, 1)
	if !turn.send(context.Background(), barrierMessage{reply: fence}) || !<-fence {
		t.Fatal("actor fence failed")
	}
	if selected.state == entryEvicted {
		t.Fatal("reserved claim was evicted")
	}
	close(backend.release)
	select {
	...
	if !turn.applyClaimOutcome(selection, true, false) {
		t.Fatal("claim outcome was not applied")
	}
	second, ok := eng.ClaimCall(context.Background(), scope, Call{
		CallID: "second", Tool: "query", Arguments: arguments,
	})
	if !ok || second != "late-handle" {
		t.Fatalf("remaining deterministic claim = %q, %v", second, ok)
	}
	_ = endTurn(t, eng, scope)
}

func TestRetractCancelsInvalidatedUnclaimedBet(t *testing.T) {
	backend := newFakeBackend()
	scope := testScope("retract")
	eng := beginTestEngine(t, backend, Config{}, scope)
	observe(t, eng, scope, "peek-1", `{"prompt":"predicted"}`, false)
	waitStarts(t, backend, 1)
	if !eng.Retract(scope, "peek-1") {
		t.Fatal("Retract rejected")
	}
	deadline := time.Now().Add(2 * time.Second)
	for {
		_, cancels := backend.counts()
		if len(cancels) == 1 {
			if cancels[0] != "h-peek-1" {
				t.Fatalf("cancelled %q, want h-peek-1", cancels[0])
			}
			break
		}
		if time.Now().After(deadline) {
			t.Fatalf("cancels = %v, want one", cancels)
		}
		time.Sleep(time.Millisecond)
	}
	metrics := endTurn(t, eng, scope)
	if metrics.Evictions != 1 || metrics.Cancelled != 1 {
		t.Fatalf("metrics = %+v, want one retracted cancellation", metrics)
	}
	if _, ok := eng.Claim(context.Background(), scope, "query", []byte(`{"prompt":"predicted"}`)); ok {
		t.Fatal("retracted bet remained claimable")
	}
}
