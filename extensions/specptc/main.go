// Command reasonix-spec-ptc provides a combined RLM and streamed speculative
// PTC runtime, plus the Extension Protocol speculation strategy used by native
// Reasonix tool calls.
package main
...
func (b *hostBackend) Cancel(ctx context.Context, handle engine.Handle) error {
	...
	b.mu.RUnlock()
	if !bound {
		return errors.New("specptc: host is not bound for cancellation")
	}
	result, err := host.Cancel(ctx, extension.HostSpeculationCancelParams{
		Scope:  scope,
	...
	if !result {
		return errors.New("specptc: host did not confirm cancellation")
	}
	b.handlesMu.Lock()
	if b.scopes[handle] == scope {
...
func (b *hostBackend) claim(handle engine.Handle) {
	b.handlesMu.Lock()
	delete(b.scopes, handle)
	delete(b.completed, handle)
	b.handlesMu.Unlock()
...
func (p *plugin) Initialize(_ context.Context, params extension.InitializeParams) (*extension.InitializeResult, error) {
	...
	if p.provider != nil && p.provider.runtime != nil {
		result.Providers = []extension.ProviderDescriptor{rlmProviderDescriptor(p.provider.model)}
	}
	return result, nil
}
...
func (p *plugin) shutdown(context.Context) {
	...
	if eng != nil {
		if err := eng.Close(); err != nil {
			if p.log != nil {
				p.log.Printf("speculation shutdown left unresolved host cancellations: %v", err)
			}
			return
		}
	}
...
func main() {
	...
	provider := newRLMProvider(logger, cfg)
	p := &plugin{
		log: logger, policies: policies, backend: backend, provider: provider,
		cfg: cfg,
	}
	err := extension.Serve(context.Background(), p, extension.Options{
		Name: "spec-ptc", Version: "0.2.0",
		Provider: provider,
